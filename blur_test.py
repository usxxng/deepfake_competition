...
python blur_test.py --kernel-type 5fold_b5_30ep_alb01 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 10 --batch-size 16 --k-fold 5 --image-size 456
...
def predict_model(model, attack_strength):
    ...
    transforms_train, transforms_val = get_transforms(args.image_size, attack_strength)
    ...
    df_test[['image_name', 'target']].to_csv(
        os.path.join(args.sub_dir, f'sub_{args.kernel_type}_{args.eval}_{0}_{acc:.2f}_{auc:.4f}.csv'), index=False)
...
def main():
    ...
    for fold in folds:
        ...
        for attack_strength in [7,11,15,21,25,27,31]:
            result[cnt] = predict_model(model, attack_strength)
            print(f'as: {attack_strength}, auc: {result[cnt]}')
            cnt+=1



        print("blur Result : ", result)
