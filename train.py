...
python train.py --kernel-type 5fold_b5_20ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 20 --batch-size 64 --CUDA_VISIBLE_DEVICES 0,1

python train.py --kernel-type 2fold_b5_10ep_alb01 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 10 --batch-size 16 --k-fold 2
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, target_idx):
    ...
    if os.path.isfile(model_file3):
        ...
        model.load_state_dict(torch.load(model_file)) # 모델 경로 지정
    else:
        model = ModelClass(
            args.enet_type,
            n_meta_features=n_meta_features,
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        # else:
        #     val_loss, acc, auc, auc_no_ext = [999.,0.,0.,0.]
